...
test('技能包发布元数据版本保持一致', async () => {
  ...
  const localReadme = path.join(repoDir, 'README.md');
  const rootReadmePath = await fs.access(localReadme)
    .then(() => localReadme)
    .catch(() => path.resolve(repoDir, '..', 'README.md'));
  const rootReadme = await fs.readFile(rootReadmePath, 'utf8');
