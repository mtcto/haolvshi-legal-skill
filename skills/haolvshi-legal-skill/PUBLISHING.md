...
这里的“托管”是指把安装文档、技能压缩包和技能图标放到官网，供用户或人工智能工具下载安装；它与法律咨询、计算器、合同审核等应用的在线地址无关。服务器托管只是完整发布的一部分，完整发布还必须同步 GitHub、ClawHub 和 Claude。
...
命令行安装可以使用官网压缩包地址。正式发布仍需按仓库根目录 `AGENTS.md` 和 `发布平台说明.md` 同步 GitHub 公开仓库与 Release、ClawHub 和 Claude。WorkBuddy 两类专用包只在本地生成和校验，不自动打开页面或上传。
